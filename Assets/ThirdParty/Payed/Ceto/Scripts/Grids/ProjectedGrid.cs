...
namespace Ceto
{
	...
	public class ProjectedGrid : OceanGridBase
    {
		...
		/// </summary>
		Dictionary<MESH_RESOLUTION, Grid> m_grids = new Dictionary<MESH_RESOLUTION, Grid>();
		...
		void Start()
		{
			...
			try
			{
				...
				if(m_ocean.UnderWater != null && m_ocean.UnderWater.Mode == UNDERWATER_MODE.ABOVE_AND_BELOW && oceanUnderSideMat == null)
        ...
        void ApplyProjection(GameObject go)
        {
			...
			try
			{
                ...
                if (!data.projection.updated)
				{
					...
					m_ocean.Projection.UpdateProjection(cam, data, m_ocean.ProjectSceneView);
        ...
        void CreateGrid(MESH_RESOLUTION meshRes)
        {
            ...
            if(!m_grids.TryGetValue(meshRes, out grid))
            {
                grid = new Grid();
                m_grids.Add(meshRes, grid);
            }
