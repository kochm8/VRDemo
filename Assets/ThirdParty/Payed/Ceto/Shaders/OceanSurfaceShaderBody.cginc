...
void OceanSurfTop(Input IN, inout SurfaceOutputOcean o) 
{
	...
	float3 worldDepthPos = WorldDepthPos(screenUV.xy);

	fixed3 caustics = CausticsFromAbove(screenUV.xy, norm2, worldPos, worldDepthPos, dist);
	
	fixed3 sea = OceanColorFromAbove(norm2, screenUV, worldPos, depth, dist, caustics);
