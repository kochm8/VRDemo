...
*/
fixed3 AboveRefractionColor(float2 grabUV, half3 distortion, float3 surfacePos, float depth, fixed3 caustics)
{

	grabUV += distortion.xz;
	grabUV = saturate(grabUV);

	fixed3 grab = tex2D(Ceto_RefractionGrab, grabUV).rgb * Ceto_RefractionIntensity;
...
*/
fixed3 OceanColorFromAbove(half3 normal, float4 screenUV, float3 surfacePos, float surfaceDepth, float dist, fixed3 caustics)
{
	...
	fixed3 col = Ceto_DefaultOceanColor;
		...
		float distortionFade = 1.0 - saturate(dist * 0.01);

		half3 distortion = normal * Ceto_RefractionDistortion * distortionFade;
		...
		fixed3 refraction = AboveRefractionColor(screenUV.zw, distortion, surfacePos, depthBlend, caustics);
...
*/
fixed3 SkyColorFromBelow(half3 normal, float4 screenUV, float3 surfacePos, float surfaceDepth, float dist)
{
...
float EdgeFade(float2 screenUV, float3 view, float3 surfacePos, float3 worldDepthPos)
{
	...
	//Fade based on dist between ocean surface and bottom
		...
		float oceanDepth = tex2D(Ceto_OceanDepth, screenUV).x * Ceto_MaxDepthDist;
...
*/
fixed3 CausticsFromAbove(float2 screenUV, half3 normal, float3 surfacePos, float3 worldDepthPos, float dist)
{
	...
	float2 uv = worldDepthPos.xz * Ceto_CausticTextureScale.xy + normal.xz * Ceto_CausticTextureScale.z;
	...
	float depthFade = exp(-max(0.0, surfacePos.y - worldDepthPos.y) * depthFadeScale);
...
fixed3 CausticsFromBelow(float2 screenUV, half3 normal, float3 worldDepthPos, float dist)
{
	...
	float2 uv = worldDepthPos.xz * Ceto_CausticTextureScale.xy + normal.xz * Ceto_CausticTextureScale.z * 0.5;
